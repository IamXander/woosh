...
// use crate::lib::build::bazel::remote::execution::v2::ExecuteRequest;

// use pb::{echo_client::EchoClient, EchoRequest};

// fn echo_requests_iter() -> impl Stream<Item = lib::build::bazel::remote::execution::v2::ExecuteRequest> {
//     tokio_stream::iter(1..usize::MAX).map(|i| lib::build::bazel::remote::execution::v2::ExecuteRequest {
//         message: format!("msg {:02}", i),
//     })
// }

async fn streaming_echo(client: &mut lib::build::bazel::remote::execution::v2::execution_client::ExecutionClient<Channel>, num: usize) {
    let stream = 
    client.execute(lib::build::bazel::remote::execution::v2::ExecuteRequest {
            // message: "foo".into(),
            instance_name: "yeet".into(),
    ...
    while let Some(item) = stream.next().await {
        println!("\treceived: {}", item.unwrap().name);
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut client = lib::build::bazel::remote::execution::v2::execution_client::ExecutionClient::connect("http://[::1]:50051").await.unwrap();
